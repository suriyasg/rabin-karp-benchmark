...
int main() {
    ...
    int errorsDet = 0, errorsRandB = 0, errorsRandM = 0;

    for (const auto& test : tests) {        
        // Randomized Modulo
        auto start = high_resolution_clock::now();
        int resRandM = rabinKarpRandomizedModulo(text, test.word);
        timeIntervels.push_back(duration<double>(high_resolution_clock::now() - start).count());
        if (resRandM != test.expected_index) errorsRandM++;
    }
    ...
    cout << "[Randomized Modulo (Las Vegas)]\n";
    cout << "Time:   " << totalTime << " seconds\n";
    cout << "Errors: " << errorsRandM << "\n\n";

    // Save individual times to a CSV file
    ofstream resultsFile("../result/benchmark_random_modulus.csv");
    if (resultsFile.is_open()) {
        ...
        resultsFile.close();
        cout << "Individual test times saved to result/benchmark_random_modulus.csv\n";
    } else {
        cerr << "Error: Could not create result/benchmark_random_modulus.csv\n";
    }
